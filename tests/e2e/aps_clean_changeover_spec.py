...
APS_E2E=1 APS_E2E_DETAIL=1 uv run python -m unittest -v tests.e2e.aps_clean_changeover_spec
"""
...
class ApsCleanChangeoverE2E(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        await automation.connect_browser(os.getenv("APS_CDP_URL", "http://127.0.0.1:9222"))
        context = await automation.page_context()
    ...
    async def test_page_controls_and_main_vtable(self) -> None:
        context = await automation.page_context()
        controls = await automation.analyze_scope(max_controls=30)
        analysis = await automation.vtable_analysis(
    ...
    async def test_view_opens_modal_with_nested_vtable(self) -> None:
        ...
        try:
            ...
            nested = await automation.vtable_analysis(mode="full", sample_rows=1)
            ...
            if not any("查看清洗改机规则" in item.get("text", "") for item in observed):
                observed.extend((await automation.scan_overlays(scope="active")).get("overlays", []))
            self.assertTrue(
            ...
            await automation.dom_interact(
                "press", css="body", key="Escape", frame="active", settle_ms=250
            )

    async def test_repeated_vtable_click_has_visual_evidence(self) -> None:
        first = await automation.click_cell(2, 1, verify=True)
        second = await automation.click_cell(2, 1, verify=True)
...
class ApsMaterialSubstituteDetailE2E(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_two_vtables_and_regular_form_controls(self) -> None:
        directory = await automation.vtable_analysis()
        controls = await automation.analyze_scope(max_controls=50)
    ...
    async def test_blank_save_emits_validation_message(self) -> None:
        controls = await automation.analyze_scope(max_controls=60)
        save = next(
            item
            for item in controls["controls"]
            if item["role"] == "button"
