...
from ..overlay.enrichment import _enrich_overlay_items
...
async def _dom_snapshot_impl(
    *,
    ...
    mode_arg = "ai" if ai_mode else "raw"
    target = target_frame.locator(selector) if selector else target_frame.locator(":root")
    kwargs: dict[str, Any] = {}
    if depth is not None and depth > 0:
        kwargs["max_depth"] = depth
    snapshot = await target.aria_snapshot(**kwargs) if not boxes else await target.aria_snapshot(**kwargs)
    return {
        "status": "ok",
        "selector": selector,
        "frame": frame,
...
async def _analyze_scope_impl(
    *,
    ...
    overlays = overlays_resp.get("overlays") or []
