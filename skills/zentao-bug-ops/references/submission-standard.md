...
**标题【强制铁律】**：必须以【模块名称】作为开头，格式严格遵循 `【模块】验证点：现象短语`（例如：`【插单申请】编辑返回：点击「返回」并确认后未关闭“插单申请-编辑”页签且未切回列表页签`），**严禁使用“菜单路径→页面”等任何其他前缀**。菜单路径一律在正文的【重现步骤】或【环境说明】中体现。

**正文【强制覆盖式提交 / 严禁老旧占位符】**：
1. **全量覆盖**：每次提交必须全量覆盖正文内容，由调用方提供的纯净 Markdown/HTML 根因级骨架完全主导。
2. **严禁老旧占位符**：**正文中绝对不要出现或遗留 `[步骤]`、`[结果]`、`[期望]` 这类老式占位标记**（严禁在正文头部出现 `<p>[步骤]</p>` 或文本墙）。
3. **标准骨架**（根因级结构）：
```
【环境说明】  …环境、页签、数据基线（条数）
【一句话结论】 …核心现象一句话概括
【重现步骤】  1. …  2. …
【实测表现与对照矩阵】  HTML table 对比
【现场实捕 DOM 状态证据】  <pre> 切片
【期望表现】  …正确态与期望行为
【关键排除】  …已排除的相邻项与反向证据
【根因链路推导】  <pre> 交互触发 → 逻辑错误 → 状态脱节/死锁
【业务影响】  …直接危害与波及面
【修复建议】  HTML table 方案矩阵
【回归断言点】  <ol> 判定断言
【证据截图】  <img> 内嵌截图
```
