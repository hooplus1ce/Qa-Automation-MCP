...
async def _frame_geometry(page: Page) -> dict[str, Any]:
    """一次计算全部 frame 的偏移与元素属性,供多次 enrich 共享。"""
    try:
    ...
    main_id = _frame_id(page, page.main_frame)
...
async def _enrich_overlay_items(
    ...
    *,
    ...
    active_id = str(geometry["active_id"])
    main_id = str(geometry["main_id"])
    offsets: dict[str, dict[str, float]] = geometry["offsets"]
    frame_elements: dict[str, dict[str, str]] = geometry["frame_elements"]
    ...
    for item in sorted(_dedupe_overlays(items), key=_overlay_sort_key):
        ...
        record["scope"] = (
            ...
            "top_document" if frame_id == main_id else
            "iframe"
        )
